  ...
  return (
    <div className="utils-dropdown">
      {options.map((dropbox) => {
        return (
          <>
            <div className="dropdown-btn" onClick={(e)=>setIsActive(!isActive)}>{dropbox.type} <span><ArrowDropDownIcon/></span></div>
            {dropbox.choice.map((option,index) => {
              return (<>
                {isActive && (<div className="dropdown-content">
                  <div className="dropdown-item" onClick={()=>{handleClick(index)}}>{option.opt}</div>
                </div>)}
              </>);
            })}
          </>
